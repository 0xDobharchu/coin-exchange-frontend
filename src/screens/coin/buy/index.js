...
import * as gtag from 'src/services/ga/ga-utils';
import taggingConfig from 'src/services/ga/tagging-config';
...
import AtmCashTransferInfo from 'src/components/dump';
import getErrorMessageFromCode from 'src/utils/getMessageFromCode';
...
class BuyCryptoCoin extends React.Component {
  ...
  updatePhoneNumber = (phone) => {
    if (phone) {
      this.props.rfChange(nameBuyCryptoForm, 'phone', phone);
    }
  }
  ...
  onMakeOrderSuccess = () => {
    ...
    if (order.type === PAYMENT_METHODS.COD) {
      ...
      //   type: 'success',
      //   callBack: () => {
      //     this.props.history.push(`${URL.HANDSHAKE_ME}?id=22`);
      //   },
      // });
      alert('Success');
      this.props.history.push(`${URL.HANDSHAKE_ME}?id=22`);
  ...
  checkUserVerified = (info = {}) => {
    const { authProfile: { idVerified }, idVerificationLevel, fiatAmountOverLimit, fiatLimit, coinMoneyExchange } = this.props;
  ...
  renderOrderInfo = () => {
    const { modalContent, modalTitle, showModal } = this.state;
    return (
  ...
  noticeBuyPackageWithoutWalletAddress = () => {
    ...
    if (!walletAddress) {
      ...
      //   type: 'danger',
      // });
      alert('buy_package_without_wallet_address_notice');
    ...
    return (
      <div className="cod-form-container">
        <Field
          ...
          validate={paymentMethod === PAYMENT_METHODS.COD ? [inputValidator] : null}
        />
        <Field
          type="text"
          className="form-control input-field cod-field"
            ...
            packageDatas?.map(({ intlKeyName, name, packageData }) => {
              ...
              return packageData && (
                <div key={name} className={`package-item ${name}`}>
                  <span className="name">messages.buy_coin.label[intlKeyName] {intlKeyName}</span>
    ...
    return (
      <div className={className}>
        <div className={`discover-overlay ${this.state.isLoading ? 'show' : ''}`}>
          <img src={loadingSVG} alt="loading" width="100" />
        </div>
        <div className={styles.container}>
          <div className={styles.specificAmount}>
