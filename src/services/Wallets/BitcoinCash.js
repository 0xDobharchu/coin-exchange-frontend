...
export class BitcoinCash extends Bitcoin {
  ...
  getAPIUrlTransaction(transaction_no) {
    const url = `https://${bitcore.Networks.defaultNetwork === bitcore.Networks.livenet ? '' : 'test-'}bch-insight.bitpay.com/#/tx/${transaction_no}`;
    return url;
  }

  getAPIUrlAddress() {
    const url = `https://${bitcore.Networks.defaultNetwork === bitcore.Networks.livenet ? '' : 'test-'}bch-insight.bitpay.com/address/${this.address}`;
    return url;
  }
  ...
  createAddressPrivatekey() {
    ...
    // get Cashaddr
    const address = new bitcore.PrivateKey(this.privateKey).toAddress();
  ...
  async getBalance(isFormatNumber) {
    ...
    if (response.status == 200) {
      if (isFormatNumber) { return this.formatNumber(response.data); }
