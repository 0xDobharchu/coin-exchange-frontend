...
export class Ethereum extends Wallet {
  ...
  async getBalance(isFormatNumber) {
    try {
      ...
      if (isFormatNumber) { return this.formatNumber(Web3.utils.fromWei(balance.toString())); }
  ...
  async getBalancePending() {
    try {
      ...
      const result = await this.getTransactionHistory(1, 5);
      ...
      if (result) {
        ...
        for (let i = 0; i < result.length; i++) {
          ...
          if (item.isError === '0' && item.confirmations === '0') {
            totalBalance -= (Number(item.gasPrice) + Number(item.value));
  ...
  getGasPrice = async (speed = 1) => new Promise((resolve, reject) => {
  ...
  getLevelFee = async () => new Promise((resolve, reject) => {
    ...
    if (result && result.length) {
      ...
      result = [];
      const web3 = this.getWeb3();
      const calcGasTimeFee = (data, title, min) => {
        try {
          const gasPrice = Number(data / 10);
          const estimatedGas = defaultGasLimit * (new BN(gasPrice * 1000000000));
          let ethEstimateGas = Number(web3.utils.fromWei(estimatedGas.toString()));
          const feePrice = ethEstimateGas;
          ethEstimateGas = this.formatNumber(ethEstimateGas, 8);

          return {
            title, description: `${ethEstimateGas} ETH ~ ${min} min${min > 1 ? 's' : ''}`, value: gasPrice.toString(), feePrice
          };
        } catch (e) {
          console.error(e);
        }
  ...
  async transfer(toAddress, amountToSend, opt = {}) {
    const data = opt.data || '';
    let fee = opt.fee || 0;
    const gasLimit = opt.gasLimit || defaultGasLimit;
    ...
    try {
      ...
      const gasPrice = new BN(Number(fee) * 1000000000);// converts the gwei price to wei;
  ...
  async getNonce(accountAddress) {
    ...
    const nonce = await web3.eth
      ...
      .then((_nonce) => {
        this.lastResultNonce = this.lastResultNonce >= _nonce ? this.lastResultNonce + 1 : _nonce;
  ...
  getAPIUrlTransaction(hash) {
    const url = this.network == Ethereum.Network.Mainnet ? `https://etherscan.io/tx/${hash}` : `https://rinkeby.etherscan.io/tx/${hash}`;
    return url;
  }
  ...
  cook(data) {
    let value = 0; let transaction_date = new Date(); const addresses = [];


    let is_sent = 0; let is_error = false; let transaction_no = ''; let token = {}; let
      coin_name = 'ETH';
    ...
    if (data) {
      try {
        ...
        if (String(data.from).toLowerCase() == this.address.toLowerCase() && String(data.to).toLowerCase() != this.address.toLowerCase()) { is_sent = 1; } else if (String(data.from).toLowerCase() != this.address.toLowerCase() && String(data.to).toLowerCase() == this.address.toLowerCase()) { is_sent = 2; }
      } catch (e) {
        console.error(e);
      }
  ...
  async getTransaction(hash) {
    ...
    if (response.status == 200) {
      ...
      result.value = Web3.utils.hexToNumberString(result.value);      
  ...
  cookIT(data) {
    let value = 0; let transaction_date = new Date(); let toAddress = ''; let is_error = false;


    let transaction_no = ''; let is_sent = 0; const
      addresses = [];
    ...
    if (data) {
      try {
        ...
        if (data.type == 'create') { is_sent = 3; } else if (String(data.from).toLowerCase() == this.address.toLowerCase() && String(data.to).toLowerCase() != this.address.toLowerCase()) { is_sent = 1; } else if (String(data.from).toLowerCase() != this.address.toLowerCase() && String(data.to).toLowerCase() == this.address.toLowerCase()) { is_sent = 2; }
